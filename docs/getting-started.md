...
Now that you understand the input and output, let's implement `reconciler.sh`. Implement a reconciler that outputs the value of the `.spec.message` field to stderr and sets the valud `completed` to the `.status.phase` field.
